"""AlphaLDM with Pareto BO over train RankIC and train RankICIR.

The upstream small-molecule case2 task models its two assay objectives with one
independent GP each and selects from the generated reservoir using Monte Carlo
expected hypervolume improvement (EHVI).  This method ports that decision rule
to the existing AlphaResearch fingerprint space:

    GP_rank_ic(X), GP_rank_icir(X) -> EHVI(Pareto(D), reference) -> softmax pick

Both objectives use training data only.  Validation still selects the final pool
on the configured protocol objective (formally RankIC), and Test remains report
only, so this changes search without weakening the existing leakage boundary.
"""
...
OBJECTIVES = ("rank_ic", "rank_icir")
MAXIMIZE = (True, True)


class RankICRankICIRHistory(History):
    """Base-compatible history with both realized objectives persisted."""
...
class RankICRankICIRSurrogate:
    ...
    def fit(self, features: np.ndarray, _primary_scores: np.ndarray) -> None:
        ...
        if len(objective_scores) != len(features):
            raise ValueError("dual-objective scores and fingerprint history are misaligned")
...
class RankICRankICIREHVILDM(AlphaLDM):
    """Seeded AlphaLDM whose proposal pool is acquired by two-objective EHVI."""

    name = "ldm_rankic_rankicir_ehvi"
    candidate_source = "ldm_rankic_rankicir_ehvi"
    ...
    def __init__(
        ...
        *,
        ...
        ehvi_reference_rank_icir: float = -0.50,
        ...
        **kwargs: Any,
        ...
        if objective != "rank_ic":
            raise ValueError(
                "ldm_rankic_rankicir_ehvi keeps the formal validation/test objective "
                f"at rank_ic, got {objective!r}"
            )
        search_objective = kwargs.get("search_objective")
        if search_objective not in {None, "rank_ic"}:
            raise ValueError(
                "ldm_rankic_rankicir_ehvi fixes its search objectives to rank_ic and "
                f"rank_icir, got scalar override {search_objective!r}"
            )
        reference = (
            float(ehvi_reference_rank_ic),
            float(ehvi_reference_rank_icir),
        )
        ...
        if requested_acquisition != "ehvi":
            raise ValueError(
                "ldm_rankic_rankicir_ehvi requires acquisition='ehvi', got "
    ...
    def _objective_values(result: EvaluationResult) -> tuple[float, float]:
        return (
            ...
            result.metrics.value("rank_icir"),
        )
    ...
    def _new_history(self, *, dim: int, schema_version: str) -> RankICRankICIRHistory:
        return RankICRankICIRHistory(dim=dim, schema_version=schema_version)
    ...
    def _new_surrogate(
        ...
        *,
        feature_dim: int,
        history: History,
    ) -> RankICRankICIRSurrogate:
        if not isinstance(history, RankICRankICIRHistory):
            raise TypeError("two-objective method requires RankICRankICIRHistory")
        return RankICRankICIRSurrogate(
    ...
    def _record_observation(
        ...
        *,
        ...
        if not isinstance(history, RankICRankICIRHistory):
            raise TypeError("two-objective method requires RankICRankICIRHistory")
        rank_ic, rank_icir = self._objective_values(result)
    ...
    def _evaluated_record(
        ...
        *,
        ...
        return {
            "expression": expression,
            "score": score,
            "scores": {"rank_ic": rank_ic, "rank_icir": rank_icir},
        }

    def _generator_objective(self) -> str:
        return "joint RankIC and RankICIR Pareto improvement"
    ...
    def _candidate_acquisition(
        ...
        *,
        ...
        if not isinstance(history, RankICRankICIRHistory):
            raise TypeError("two-objective method requires RankICRankICIRHistory")
        if not isinstance(surrogate, RankICRankICIRSurrogate):
            raise TypeError("two-objective method requires RankICRankICIRSurrogate")
        if not history.n:
            raise RuntimeError("EHVI requires the seeded RankIC/RankICIR warm-up history")
        ...
        return means[0], standard_deviations[0], values, {
            "posterior_mean_rank_icir": means[1],
            "posterior_std_rank_icir": standard_deviations[1],
        }
    ...
    def _warmup_event_fields(
        ...
        rank_icir_cv = _cross_validated_rank_correlation(
            history.features(),
            points[:, 1],
            self.gp_kwargs,
        )
        return {
            ...
            "cv_rank_correlation_rank_icir": rank_icir_cv,
        }
